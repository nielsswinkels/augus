...
- **Translate admin interface:** The admin UI is currently English-only. Add an i18n system similar to the visitor app so the admin can be used in other languages. Useful for museum staff who don't speak English fluently. Could auto-detect from browser language or have a setting.
...
- **Multiple objects at the same GPS location:** Outdoor exhibitions may have several objects at the same physical spot (e.g. a viewpoint with multiple things to talk about, or a monument with several stories). Currently all pins stack on top of each other and GPS auto-trigger can only fire one. Needs design work — possible approaches: auto-cluster same-location pins into a single tappable pin that expands a list; allow an object to have multiple "chapters" or sections instead of requiring separate objects; offset pins slightly in a circle around the actual point; or treat co-located objects as a group that opens as a mini-list when tapped. Also affects which audio to auto-play when the visitor walks into the trigger radius. Unsolved — needs real-world testing to find the right UX.
- **Parallel exhibitions on the same route:** Multiple sets could share the same physical objects/locations — e.g. an informational tour and a quiz/tipspromenad running simultaneously. Currently visitors can only be in one set at a time. Needs design work: should visitors be able to "subscribe" to multiple sets and switch between them per object? Or should sets be layerable (one base set + optional overlay sets)? Also relevant for events where a temporary exhibition adds content alongside a permanent one. Affects QR codes (which set does the QR point to?), the welcome page, and navigation.
- **Adult/kid content tracks:** Some exhibitions want different content for different audiences (adult vs. kid-friendly descriptions, different audio). Using languages for this ("English", "English (Kids)") technically works but feels wrong in the UI — a language dropdown shouldn't mix audience selection. Possible approaches: a separate "content track" or "audience" selector in settings (only shown when the set has multiple tracks); a per-object toggle between tracks; or just multiple audio files per language with a label. Needs design — should only appear in the UI when the content actually exists for multiple audiences.
- **Quiz / tipspromenad mode (low priority):** Add optional interactive quiz questions to objects. After listening to the audio, a question appears ("What year was this built?") with multiple-choice answers. Could combine with treasure hunt mode for a gamified experience. Track score in localStorage, show a results screen at the end. Very much feature creep territory — only worth doing if there's real demand from a museum partner.
