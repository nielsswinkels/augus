...
- **Localized floor labels:** Floor short labels (e.g. "G", "1", "2") should support per-language values. For example, ground floor is "G" in English but "E" (entréplan) in Swedish. Currently the label is a single field — needs `label_en`/`label_sv` (or dynamic per the flexible language system) so the floor selector buttons show the correct label for the visitor's language.
...
- **Thumbnail image sizes:** Generate low-resolution versions of images to use for list view thumbnails and carousel slides. Currently the full-size image is loaded even for small thumbnails, wasting bandwidth — especially on museum WiFi. PocketBase supports thumb generation via URL params (e.g. `?thumb=200x200`), or we could generate and store small versions on upload.
